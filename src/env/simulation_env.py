...
import torch
from src.agents.llm_analyzer import SemanticAnalyzer
try:
...
try:
    from visualization.gui import SimulationGUI
    GUI_ENABLED = True
except ImportError:
    import sys
...
class EdgeServer:
    ...
    def process_task(self, task):
        ...
        with self.resource.request() as req:
            ...
            self.remaining_energy = max(0.0, self.remaining_energy - energy)
...
class IoTDevice:
    ...
    def run(self):
        while True:
            ...
            # 2. Task Generation (Poisson Process) - âœ… INCREASED RATE for faster testing
            ...
            if LLM_ANALYZER and getattr(LLM_ANALYZER, "model", None):
                ...
                # # LLM yÃ¼kleniyor, rule-based ile analiz et
            ...
            # âœ… VERBOSE LLM Recommendation with detailed reasoning
            ...
            l1 = f"LLM Analizi: {llm_rec} Ã¶neriliyor ({llm_reason}) [GÃ¼ven: {llm_confidence:.0%}]"
            
            # âœ… VERBOSE PPO Decision with detailed reasoning
            if final_decision_idx == 5:
                l2 = f"PPO Karar: Bulut tercih. Sebep: SNR dÃ¼ÅŸÃ¼k ({snr_db:.1f}dB), veri bÃ¼yÃ¼k ({task.size_bits/1e6:.1f}MB), Edge yoÄŸun ({closest_edge.queue_length} task bekliyor)."
            elif final_decision_idx == 4:
                l2 = f"PPO Karar: Full Edge. Sebep: Orta iÅŸlem ({task.cpu_cycles/1e9:.2f}GHz), iyi sinyal ({snr_db:.1f}dB), Edge uygun durumda."
                ...
                l2 = f"PPO Karar: Full Local. Sebep: Basit gÃ¶rev ({task.cpu_cycles/1e9:.2f}GHz), batarya korumasÄ± ({self.battery/BATTERY_CAPACITY*100:.0f}%), aÄŸ tasarrufu."
            ...
            if is_conflict:
                ...
                override_msg = f"âš ï¸ NÃ¶ral Ã‡eliÅŸki: LLM Ã¶nerisi ({rec.upper()}), PPO tarafÄ±ndan ({ppo_simple}) olarak seÃ§ildi. FarklÄ± stratejiler uygulanÄ±yor."
                multi_line_msg = f"{l1}\n{l2}\n{l3}\n{override_msg}"
            else:
                ...
                if 1 <= final_decision_idx <= 3:
                    ...
                    multi_line_msg += f"\nâœ“ Uyumlu: LLM ({rec.upper()}) + PPO (PARTIAL) â†’ Batarya koruma stratejisi"
...
def main():
    ...
    while env.now < until:
        ...
        # âœ… Update LLM stats every frame (not just on task completion)
    ...
    if gui:
        ...
        gui.show_toast("ğŸ SÄ°MÃœLASYON TAMAMLANDI", duration=300)
