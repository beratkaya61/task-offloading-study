...
- data regime is then made explicit in file names such as `synthetic_*`, `synthetic_trace_*`, `real_data_*`, or `real_composite_trace_*`
...
4. Run trace-driven RL training: `python experiments/phase_6/run_trace_training.py --config configs/phase_6/synthetic_trace_rl_training.yaml`
5. Run real-data ablation rerun: `python experiments/phase_5/run_real_data_ablation_study.py`
6. Experiment seed (`seed`), specific configurations, and models are deterministic via `src/utils/reproducibility.py`.
7. Inspect artefacts: workflow-specific CSV files phase bazli olarak `results/phase_*/metrics/` altina, gorseller ise `results/phase_*/figures/` altina yazilir. Faz 5 kanonik ozet raporlari `v2_docs/phase_5/synthetic_phase_5_report.md` ve `v2_docs/phase_5/real_data_phase_5_report.md` dosyalarindadir.
