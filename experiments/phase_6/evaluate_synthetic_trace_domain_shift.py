...
DEFAULT_CONFIG = REPO_ROOT / "configs" / "phase_6" / "synthetic_trace_domain_shift_evaluation.yaml"
CSV_COLUMNS = [
    "timestamp",
    "batch_id",
    "train_domain",
...
def make_trace_env(config: dict, seed: int):
    ...
    real_data_mode = is_real_data_mode(config)
    ...
    if real_data_mode:
        assert_real_data_ready(manifest_path=manifest_path, repo_root=REPO_ROOT)
    loader = TraceLoader(trace_dir=trace_dir)
    processor = TraceProcessor(trace_dir=trace_dir, seed=seed)

    allow_saved_splits = loader.has_saved_episode_splits() and (not real_data_mode or not uses_legacy_trace_dir)
...
def main(config_path: Path = DEFAULT_CONFIG):
    ...
    csv_path = REPO_ROOT / config.get("output", {}).get("csv_path", "results/phase_6/metrics/synthetic_trace/domain_shift/trace_domain_shift_evaluation.csv")
