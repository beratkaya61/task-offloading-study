...
class TraceTrainingOrchestrator:
    ...
    def __init__(self, config_path: str = "configs/phase_6/synthetic_trace_ppo_training.yaml", 
                 seed: int = 42):
        """
        Args:
    ...
    def prepare_traces(self) -> Tuple[List[TraceEpisode], 
        ...
        if real_data_mode:
            logger.info("ðŸ”’ Real-data mode enabled; synthetic fallback is forbidden.")
        ...
        # Real-data mode may use saved splits only when they come from a dedicated non-legacy directory.
        ...
        if allow_saved_splits:
            logger.info(f"ğŸ“¦ Using saved trace episode splits from {trace_dir}")
...
if __name__ == "__main__":
    ...
    orchestrator = TraceTrainingOrchestrator(
        config_path="configs/phase_6/synthetic_trace_ppo_training.yaml",
        seed=42
    )
    orchestrator.run()
