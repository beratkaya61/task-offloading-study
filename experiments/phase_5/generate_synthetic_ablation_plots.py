...
def load_ablation_config(config_path="configs/phase_5/synthetic_ablation.yaml"):
...
def resolve_ablation_paths(config):
    ...
    csv_template = output_cfg.get(
        "csv_path_template",
        "results/phase_5/metrics/synthetic/ablation/synthetic_ablation_{algorithm}_{scope}.csv",
    )
...
def resolve_latest_ablation_csv(default_csv_path, algorithm=None, scope=None):
    search_root = os.path.dirname(os.path.dirname(default_csv_path)) or "results/phase_5/metrics"
...
def generate_ablation_visuals(config_path="configs/phase_5/synthetic_ablation.yaml", csv_path=None, algorithm=None, scope=None):
...
def list_ablation_csvs(raw_dir="results/phase_5/metrics", algorithm=None, scope=None):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="configs/phase_5/synthetic_ablation.yaml", help="Ablation config path")
